...
task autonomous()
{
	...
	while(nMotorEncoder[frontRight2] < 1100 || SensorValue[backLim] > 1700)
	{
		Err = SensorValue[myGyro];
		IErr = IErr + Err;
		moveRobot(80 - 0.6*Err - 0.0001*IErr,80 + 0.6*Err + 0.0001*IErr);
		if(time1(T1) < 1100)
		{
	...
	if(Blue)
	{
		while(nMotorEncoder[frontRight2] < 135) // was 150
		{
			motorReq[MobGoalntake] = -70;
			IErr = IErr + Err;
			Err = SensorValue[myGyro];
			moveRobot(80 - 0.8*Err - 0.0003*IErr,80 + 0.8*Err + 0.0003*IErr); // was 60
		}
	}
	else
	{
		while(nMotorEncoder[frontRight2] < 170) /// was SensorValue[leftenc]
		{
			motorReq[MobGoalntake] = -70;
			IErr = IErr + Err;
			Err = SensorValue[myGyro];
			moveRobot(90 - 0.8*Err - 0.0003*IErr,90 + 0.8*Err + 0.0003*IErr);
		}
	}
	moveRobot(0,0);
	motorReq[MobGoalntake] = 0;
	...
	if(Blue){
		clearTimer(T1);
		while(nMotorEncoder[frontRight2] < 240)
		{
			//moveClawsTo(1800);
			if(SensorValue[craneEnc] > 10)
			{
	...
	if(Blue)
	{
		while(abs(nMotorEncoder[frontRight2]) < 1100 || SensorValue[backLim] > 1700)
		{
			...
			Errd = SensorValue[myGyro] + 25; // was 10
			IErrd = IErrd + Errd;
	...
	if(Blue)
	{
		...
		while(nMotorEncoder[frontRight2] > -700 || time1(T1) < 200)
		{
			moveCraneTo(100);
			Err = SensorValue[myGyro] - 450;
			moveRobot(-70 - 0.8*Err,-70 + 0.8*Err);
		}
		moveRobot(0,0);
		while(SensorValue[myGyro] < 1100)
		{
			moveRobot(80,-80);
		}
		moveRobot(0,0);
	...
	while(abs(nMotorEncoder[frontRight2]) < 700) // was 500; haydi l raj3a ba3d ma 7att l mobile goal bil 20 pt zone
	{
		moveCraneTo(0);
	...
	if(Blue)
	{
		...
		while(SensorValue[AdjtSwitch] == 0)
		{
			...
			if(time1(T1) > 5000)  {moveRobot(0,0);while(true);}
		}
		moveRobot(0,0);
		...
		// BLUE : move backwards for 0.5 secs or until the 2 bumpers are pushed together
		clearTimer(T1);
		clearTimer(T2);
		while(true)
		{
			moveRobot(-60,-60);
			if(time1(T1) > 500 || time1(T2) > 500  /*|| (SensorValue[LeftBump] == 1 && SensorValue[RightBump] == 1)*/)
			{
				break;
			}
		}
	...
	else
	{
		...
		{
			moveRobot(-60,-60);
			if(time1(T2) > 400 || time1(T1) > 400 /*|| (SensorValue[LeftBump] == 1 && SensorValue[RightBump] == 1)*/)
			{
				break;
			}
		}
	...
	// move foreward then brake
	nMotorEncoder[frontRight2] = 0;
	while(nMotorEncoder[frontRight2] < 330)
	{
		moveRobot(70,70);
	}
	moveRobot(0,0);
	...
	if(Blue)
	{
		...
		Err = 0;
		nMotorEncoder[frontRight2] = 0;
		while(nMotorEncoder[frontRight2] < 780 && SensorValue[frontLim] > 1500)
		{
			Err = SensorValue[myGyro] + 900;
			moveRobot(50 - 0.5*Err,50 + 0.5*Err);
		}
	...
	else
	{
		...
		Err = 0;
		nMotorEncoder[frontRight2] = 0;
		while(nMotorEncoder[frontRight2] < 720 && SensorValue[frontLim2] > 1700) // was 800 badal 720
		{
			Err = SensorValue[myGyro] - 900;
	...
	// added part start
	//nMotorEncoder[frontRight2] = 0;
	//while(abs(nMotorEncoder[frontRight2]) < 50)
	//{
	//	moveRobot(-70,-70);
	//}
	//moveRobot(0,0);
	...
	//// added part extension
	//nMotorEncoder[frontRight2] = 0;
	//while(nMotorEncoder[frontRight2] < 120)
	//{
	//	moveRobot(-70,-70);
	//}
	//moveRobot(0,0);
	...
	if(Blue)
	{
		...
		while(nMotorEncoder[frontRight2] < 800)
		{
			...
			motorReq[MobGoalntake] = -30;
			Err = SensorValue[myGyro] + 1200; //was 1100
			moveRobot(60 - 0.5*Err,60 + 0.5*Err);
		}
	}
	else
	...
	else
	{
		...
		moveRobot(0,0);
		nMotorEncoder[frontRight2] = 0;
		while(nMotorEncoder[frontRight2] < 10)
		{
			moveClawsTo(1800);
			Err = SensorValue[myGyro] - 1000;
			IErr = IErr + Err;
			moveRobot(75 - 0.3*Err,50 + 0.3*Err);
			//moveRobot(60,0);
		}
	}
	moveRobot(0,0);
...
task usercontrol()
{
	...
	while (true)
	{
		...
		test1 = SensorValue[frontRight2];
		test3 = SensorValue[frontLim];
