...
task autonomous()
{
	...
	//turn right 185 degree to golf the first,high flag
	...
	while(time1(T1)<2000){
		...
		IErr = IErr + Err;
		moveRobot(40 + 0.4*Err*0+ 0.0001*IErr*0,45- 0.4*Err*0 - 0.0001*IErr*0);
	}
	moveRobot(0,0);
	delay(100);

	//go back to red from the first flag post
	...
	moveRobot(0,0);
	delay(50);
	//go straight little to turn left and invert the first cap
	nMotorEncoder[backRight]=robotEncoder=0;
	...
	//turn left 80 degree to invert the first cap
	SensorValue[myGyro]=0;
	...
	invertCap();
	clearTimer(T3);
	while((robotEncoder<1100)&&(time1(T3)<5000)
		){
	...
	while(((SensorValue[leftTouch]!=1) ||( SensorValue[rightTouch]!=1))&&(time1(T1)<3000)){
		Err = SensorValue[myGyro];
		IErr = IErr + Err;
		moveRobot(-50 + 0.1*Err+ 0.0001*IErr,-50 -0.1*Err - 0.0001*IErr);
	}
	moveRobot(0,0);
	delay(50);
	...
	//turn left 90 degree to get the second line
	...
	//go straight to touch the red to get the second line
	nMotorEncoder[backRight]=robotEncoder=0;
	IErr=0;
	...
	while(robotEncoder<100){
		robotEncoder=	nMotorEncoder[backRight];
		Err = SensorValue[myGyro]+900;
		IErr = IErr + Err;
		moveRobot(40 + 0.4*Err*0+ 0.0001*IErr*0,50 - 0.4*Err*0 - 0.0001*IErr*0);
	}
	moveRobot(0,0);
	delay(50);

	//turn right 90 degree to adjust direction on second line
	...
	////turn right 62 degree to adjust position for shooting the high ,second flag
	...
	//	//turn right 20 degree to adjust position turning your back to the parking
	//while(SensorValue[myGyro] <800)
	//{
	//	moveRobot(-40,+40);
	//}
	...
	////turn left 90 degree to turn your back to the field wall
	...
	//go back to setup the gyroto  go to parking
	IErr=0;
	clearTimer(T3);
	...
	nMotorEncoder[backRight]=robotEncoder=0;
	IErr=0;
	while(robotEncoder<120){
		robotEncoder=	nMotorEncoder[backRight];
		Err = SensorValue[myGyro];
		IErr = IErr + Err;
		moveRobot(40 + 0.6*Err+ 0.0001*IErr,40 - 0.6*Err - 0.0001*IErr);
	}
	moveRobot(0,0);
	delay(50);


	//turn left 90 degree to attend parking
	while(SensorValue[myGyro] >-750)
	{
		moveRobot(40,-40);
	}
	moveRobot(0,0);
	...
	//go straight on the red tile to the parking
	nMotorEncoder[backRight]=robotEncoder=0;
	IErr=0;
	while((SensorValue[lFLeft]<1000) ||( SensorValue[lFRight]<1000)){
		robotEncoder=	nMotorEncoder[backRight];
	...
	while(robotEncoder>-300){
		robotEncoder=	nMotorEncoder[backRight];
		Err = SensorValue[myGyro]+900;
		IErr = IErr + Err;
		moveRobot(-40 + 0.1*Err*0+ 0.0001*IErr*0,-40 -0.1*Err*0 - 0.0001*IErr*0);
	}
	moveRobot(0,0);
delay(50);

	//turn right 90 degree to look to the parking
	...
	while(robotEncoder<1500){
		...
		Err = SensorValue[myGyro];
		IErr = IErr + Err;
		moveRobot(90 + 0.6*Err+ 0.0001*IErr,90 - 0.6*Err - 0.0001*IErr);
