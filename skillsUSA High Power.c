...
task autonomous()
{
	...
	while(nMotorEncoder[frontRight2] < 150)
	{
		IErr = IErr + Err;
		Err = SensorValue[myGyro];
		moveRobot(80 - 0.8*Err - 0.0003*IErr,80 + 0.8*Err + 0.0003*IErr);
	}
	moveRobot(0,0);
	...
	nMotorEncoder[frontRight2] = 0;
	IErr = 0;
	//moving back whilw holding goal 1 then brake
	...
	// move parallel to the white line
	nMotorEncoder[frontRight2] = 0;
	clearTimer(T1);
	while(nMotorEncoder[frontRight2] > -750 || time1(T1) < 200)
	{
		Err = SensorValue[myGyro] - 450;
		moveRobot(-80 - 0.8*Err,-80 + 0.8*Err);
	}
	moveRobot(0,0);
	...
	//release mobile goal in the 20 point zone
	clearTimer(T1);
	while(time1(T1) < 900)
	{
		moveRobot(127,127);
	...
	// go back to 10 point zone
	clearTimer(T1);
	nMotorEncoder[frontRight2] = 0;
	while(abs(nMotorEncoder[frontRight2]) < 600 || time1(T1) < 200) // frontright2 was 500
	{
	...
	while(nMotorEncoder[frontRight2] < 1050)
	{
		...
		//Err = SensorValue[myGyro];
		moveRobot(70 - 0.4*Err,70 + 0.4*Err);
	}
	moveRobot(0,0);
	motorReq[MobGoalntake] = 0;
	clearTimer(T2);
	...
	//// turn 400 degrees and go straight to push cones
	//clearTimer(T1);
	//while(nMotorEncoder[frontRight2] < 600)
	//{
	//	Err = SensorValue[myGyro] + 1300;
	//	moveRobot(110-0.5*Err,110+0.5*Err); // was 70
	...
	//g3 on robot : go back a little bit
	nMotorEncoder[frontRight2] = 0;
	...
	//////////////////////////////////////////////////////////////////////////////////////// Getting Fourth Mobile Goal
	//start pushing cones back ( lraj3a l kbire ba3d telet goal ma3 tedfich l cones
	...
	{
		Err = SensorValue[myGyro] ;
		moveRobot(-90-0.5*Err,-90+0.5*Err);
	}
	moveRobot(0,0);
	...
	while(time1(T1) < 1000)
	{
		Err = SensorValue[myGyro] - 1850; // was 1950 // turn to be facing 20 pt zone
		moveRobot(-0.5*Err,0.5*Err);
	}
	moveRobot(0,0);
	clearTimer(T2);
	...
	//////////////////////////////////////////////////////////////////////////////////////// Getting Fifth Mobile Goal
	// going back then brake
	...
	while(nMotorEncoder[frontRight2] < 1050)
	{
		...
		Err = SensorValue[myGyro] - 250;
		//Err = SensorValue[myGyro];
		moveRobot(80 - 0.4*Err,80 + 0.4*Err);
